...
export function PieCard({ title, values, colors, labels, tints, currencySymbol = "S/", danger = false }: {
  title: string; values: number[]; colors: Palette; labels: string[]; tints: string[]; currencySymbol?: string; danger?: boolean;
}) {
  ...
  return (
    <View style={[styles.chartCard, { backgroundColor: colors.card }]}>
      <Text style={[styles.sectionTitle, { color: colors.text }]}>{title} - {currencySymbol} {values.reduce((a: number, b: number) => a + b, 0).toFixed(2)}</Text>
        ...
        {labels.map((label: string, i: number) => {
          ...
          return (
            <View key={label} style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
              ...
              <Text style={{ fontSize: 11, fontWeight: "700", color, fontVariant: ["tabular-nums"] }}>{currencySymbol} {values[i].toFixed(2)}</Text>
            </View>
          );
        })}
      </View>}
